javascript: (function() {
  ...
  const FS_ARTICLE =
    '.o36gj0jk:not([role="navigation"]),[role="complementary"]';
  const NEW_ARTICLE = POST_ARTICLE + "," + FS_ARTICLE + "," + VIDEO_ARTICLE;
  const POST_ACTION =
    '.sjgh65i0 > div > div > div > div > div > span,.jifvfom9[role="button"],.p24jkzn5,.sjgh65i0 > div > div > div > div > div > a > div';
  const POST_ROOT =
    ".userContentWrapper,.uiScrollableAreaContent," + NEW_ARTICLE;
  ...
  const FILTER_NEW =
    '.bp9cbjyn > .p8dawk7l[role="button"]:not([aria-label]):not([aria-haspopup]):not(.jifvfom9):not(._4sxc)';
  ...
  class CfgWindow extends BaseWindow {
    ...
    create(div) {
      ...
      node.innerHTML = "<i>ExpandAll</i>Settings";
      node.style.marginLeft = "4px";
      node.style.fontWeight = "bold";
      const boxes = [
        ["InClassicFacebook,click<i>ContinueReading</i>links.", EXPAND_POST],
        ["Expandcomments.", EXPAND_COMMENTS],
        ["Expandreplies.", EXPAND_REPLIES],
        ["Don'tforce<i>AllComments</i>filter.", EXPAND_FILTER],
      ];
      boxes.forEach((item) => {
  ...
  class Root {
    ...
    static removeOverlay() {
      document.querySelectorAll(CSS_LOGIN_STUFF).forEach((item) => {
        Global.log("Removingoverlaystuff");
    ...
    determine() {
      ...
      if (!!div) {
        ...
        if (topOnly) {
          if (
            Dom.filterHidden(document.querySelectorAll('[role="grid"]'))
              .length == 0
          ) {
            topOnly =
              Dom.filterHidden(
            ...
            Dom.filterHidden(document.querySelectorAll('[role="navigation"]'))
              .length == 2
          ) {
            topOnly = false;
          }
          ...
          topOnly =
            Dom.filterHidden(document.querySelectorAll('[role="feed"]'))
              .length > 1;
        }
        if (topOnly) {
          Global.log(EXPANDING + "Topmostpost");
      ...
      let check = [];
      check.push([".uiStreamStory", "Videocommentsonright", !USE_PARENT]);
      check.push([
        "div.rhcScroller.uiScrollableAreaContent",
        "Theatermode",
        USE_PARENT,
      ]);
      check.push([".uiLayer:not(.hidden_elem)", "Overlaidpost", !USE_PARENT]);
      check.push([".permalinkPost", "Permalinkedpost", !USE_PARENT]);
      check.push([
      ...
      check.push(['[role="main"]', "Maincontentarea", !USE_PARENT]);
    ...
    static prepIfClassicVideo(onDone) {
      ...
      if (Root.isClassicVideo()) {
        ...
        if (links.length > 0) {
          Global.log("Makingsurevideocommentsareshowing");
    ...
    isSearchResults() {
      let result = !!Global.root.query(
        '#pagelet_group_search,.p8dawk7l[role="presentation"]'
      );
      return result;
    }
    countPosts() {
  ...
  function closeShareReport(onDone) {
    const nodes = document.querySelectorAll(
      ".sx_1ea4e0,.sx_ecfa40,.sx_1564cf,.sx_4564fd,.sx_4cbb0f,.sx_04ac81,.sx_7789b4,.sx_48f6e2,.sx_05f423,.sx_153347,.sx_03453b,.sx_27de63,.sx_87f98f"
    );
    if (nodes.length === 1) {
  ...
  function ensureCommentsShowing(onDone) {
    if (Global.root.isSearchResults()) {
      Global.log("Youmustmanuallydrilldownintoasearchresult.");
      if (onDone) onDone();
      return;
    }
    let filter = [];
    ...
    if (filter.length > 0) {
      Global.log("Showingcommentareafor" + filter.length + "post(s)");
    ...
    Global.root.queryAll(POST_ROOT).forEach((item) => {
      if (!item.querySelector(selector)) {
        const link = item.querySelector(
          '.gtad4xkn:first-child.auili1gw[role="button"]'
        );
  ...
  function newWindowNow(link) {
    ...
    Global.log("Newwindow:" + anchor.textContent);
    if (
      !window.open(anchor.getAttribute("href"), anchor.getAttribute("target"))
    ) {
      Global.log("Newwindowwasblocked!");
    }
  }
  ...
  function doNotWait(value) {
    return (
      [CSS_SEE_MORE, SEE_MORE_NEW, BASE_SEE_MORE, EXPOSE_CONTENT].indexOf(
        value
      ) >= 0
    );
  }
    ...
    if (filter.length > 0) {
      if (comments) {
        filter = filter.filter(
          (item) => !item.querySelector(CLASSIC_GET_REPLIES)
        );
      } else {
    ...
    if (filter.length == 0) {
      ...
      if (filter.length > 0) {
        if (comments) {
          ...
          filter = filter.filter(
            (item) => !!item.parentNode.parentNode.querySelector(".ozuftl9m")
          );
        }
        filter = filter.filter(
          (item) =>
  ...
  function clickAndWait(value, onDone, links, i) {
    ...
    Global.log(
      "click(" + (links.length - i - 1) + "left):" + getBestLabel(links[i])
    );
    links[i].click();
    if (value == _NONE) {
  ...
  function setFilter(onDone) {
    window.filters = Array.from(
      Global.root.queryAll("[" + FILTER_ATTR + "]")
    ).filter((item) => item.getAttribute(FILTER_ATTR) != FILTER_VALUE);
    window.filters_i = 0;
    window.filters_onDone = onDone;
    if (window.filters.length > 0) {
      Global.log("Changing" + window.filters.length + "Classicfilter(s)");
    }
    filterOne();
  }
  ...
  function setFilterNew(onDone) {
    ...
    window.filters = window.filters.filter(
      (item) => !item.closest(".sq6gx45u")
    );
    window.filters = window.filters.filter(
      (item) => !item.querySelector("h1,svg")
    );
    if (window.filters > Global.root.countPosts()) {
      Global.log("Somethingwentwrong");
      Global.log("Notchecking" + window.filters.length + "Newfilter(s)");
      if (onDone) onDone;
      return;
    }
    window.filters_i = 0;
    window.filters_onDone = onDone;
    if (window.filters.length > 0) {
      Global.log("Checking" + window.filters.length + "Newfilter(s)");
    }
    filterOneNew();
  }
  ...
  function setFilterNew2(link) {
    let filter = Array.from(
      document.querySelectorAll('.ama3r5zh[role="menu"],.swg4t2nn[role="menu"]')
    );
    filter = filter.filter(
      (item) => item.querySelectorAll('[role="menuitem"]').length >= 3
    );
    if (filter.length == 1) {
  ...
  class Actions {
    ...
    doAction() {
      if (this.actions[this.i] !== null) {
        this.actions[this.i](() =>
          window.setTimeout(bind(this, this.doAction), 50)
        );
        this.i++;
      }
    }
  ...
  class Session {
    ...
    static endSession() {
      ...
      Global.ending = true;
      Global.log("[Press\u0027s\u0027nowforSettings]");
